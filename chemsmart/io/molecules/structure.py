...
class Molecule:
    ...
    def write_pdb(self, filename, residue_name="MOL", chain_id="A", **kwargs):
        """
        Write the molecule to a PDB format file.

        Args:
            filename (str): Output PDB file path
            residue_name (str): Residue name for PDB records. Default 'MOL'
        ...
        with open(filename, "w") as f:
            ...
            f.write("REMARK   Generated by chemsmart\n")
            ...
            for i, (symbol, pos) in enumerate(zip(self.chemical_symbols, self.positions)):
                ...
                # PDB ATOM record format with proper column alignment
                # Using single format string for clarity and maintainability
                line = (
                    f"ATOM  {atom_num:5d} {symbol:<4s} {residue_name:>3s} {chain_id:1s}   1    "
