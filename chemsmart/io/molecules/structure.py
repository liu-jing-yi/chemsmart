...
class Molecule:
    ...
    def write(self, filename, format="xyz", mode="w", **kwargs):
        ...
        if format.lower() == "xyz":
            ...
            self.write_pdb(filename, **kwargs)
    ...
    def write_pdb(self, filename, **kwargs):
        """
        Write the molecule to a PDB format file.
        ...
        Note:
            ...
            - ATOM records have specific column positions
            - Coordinates are in Angstroms
            - Standard PDB format for molecular structures
        """
        with open(filename, "w") as f:
            logger.info(f"Writing PDB file to {filename}")
            
            # Write header
            f.write(f"REMARK   Generated by chemsmart\n")
            f.write(f"REMARK   Formula: {self.chemical_formula}\n")
            if self.energy is not None:
                f.write(f"REMARK   Energy: {self.energy:.6f} eV\n")
            ...
            for i, (symbol, pos) in enumerate(zip(self.chemical_symbols, self.positions)):
                atom_num = i + 1
                x, y, z = pos
                # Format: ATOM with proper spacing
                # Serial number: right-aligned in columns 7-11 (5 characters)
                # Atom name: left-aligned in columns 13-16 (4 characters)
                # Residue name: right-aligned in columns 18-20 (3 characters)
                # Chain ID: column 22 (1 character)
                # Residue seq: right-aligned in columns 23-26 (4 characters)
                # X, Y, Z: right-aligned with 3 decimal places (8.3 format)
                # Occupancy and temp factor: 1.00 and 0.00 as defaults
                # Element: right-aligned in columns 77-78 (2 characters)
                
                line = (
                    f"ATOM  {atom_num:5d} {symbol:<4s} MOL A   1    "
    ...
    def to_pdb(self, output):
        """
        Save the molecule data in PDB format to a file.

        This is a convenience method that calls write_pdb.

        Args:
            output (str): Path to the output PDB file

        Example:
            >>> molecule.to_pdb(output="molecule.pdb")
        """
        self.write_pdb(output)
